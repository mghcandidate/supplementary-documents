...
# Sensitivity Analysis for Tunisia Measles Model
...
run_sensitivity <- function(param_name, param_value) {
  ...
  # Run the model
  results <- ode(y = Y, times = times, func = Tunisia_model, parms = new_params, method = "lsoda")
...
print(p11)
# Save the plot
ggsave("sensitivity_analysis_measles_Tunisia.png",plot=p11,width=12, height=10,dpi=300)
