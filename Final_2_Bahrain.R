#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#                             SIR Model for Measles for Bahrain
...
#dev.off()
# Packages
#save.image(file = "Final_0_Bahrain.RData")-->This is the saved enviroment to be more fast
#load("Final_0_Bahrain.RData")
...
pacman::p_load(
  ...
  bigmemory,  #For bigdata computation
  dtplyr      #For Speed 
)
...
(Bahrain_pop_in_thousands<-as.data.frame(Bahrain_pop|>
                                           mutate(Population_age=Population_age*1000)))
#1.c. Visualization of Bahrain's population structure
pacman::p_load(ggplot2,dplyr,plotly)
names(Bahrain_pop_in_thousands)
(pyramyd<-ggplot(Bahrain_pop_in_thousands, aes(x = Age_Category, y = Population_age)) +
    geom_col(fill = "steelblue") +
    labs(title = "Bahrain's population structure (2023)",
         x = "Age", y = "Population") +
    theme_minimal() +
...
#ggplotly(pyramyd)
# Shorten  the name
popstruc<-Bahrain_pop_in_thousands
#1.e.Number of age groups 
(A<-n_age<-length(Bahrain_pop_in_thousands[,6]))
...
(pyramyd<-ggplot(popbirth, aes(x = Age, y = Birth)) +
    geom_col(fill = "blue") +
    labs(title = "Birth in Bahrain (2023)",
         x = "Age", y = "Births") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
)
...
(popmort<-as.data.frame(mortality|>filter(Country=="Bahrain")|>
                          filter(Year==2023)))
(pyramyd<-ggplot(popmort , aes(x =Age , y = Percentage)) +
    geom_col(fill = "red") +
    labs(title = "Mortality in Bahrain (2023)",
...
(m_contact_1y <- as.matrix(read.csv("3.U.1.contact_Bahrain_1y.csv")))
dim(m_contact_1y)
...
(p<-ggplot(df, aes(x = Contactor, y = Contactee, fill = Contacts)) +
    ...
    theme_minimal() +
    labs(title = "Contact Matrix Heatmap for Bahrain")
)
#(p_plotly <- ggplotly(p))
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#           1.i. Aging
...
#Let me select Bahrain and WHO/UNICEF estimated coverage
table(coverage$NAME)
coverage<-coverage|>
  filter(COVERAGE_CATEGORY=="WUENIC")|>
  filter(NAME=="Bahrain")
#I will need to select antigens 
MCV1_Bahrain<-coverage|>
  filter(ANTIGEN=="MCV1")
MCV2_Bahrain<-coverage|>
  filter(ANTIGEN=="MCV2")
print(MCV2_Bahrain)
table(MCV2_Bahrain$YEAR)
#I will need the coverage only  
#Secondly,  i will reverse the order in order to have 2000-2023 instead 2023-2000 
cov_MCV1_Bahrain<-rev(MCV1_Bahrain[,11])
cov_MCV2_Bahrain<-rev(MCV2_Bahrain[,11])
cov_MCV1_Bahrain
cov_MCV2_Bahrain
length(cov_MCV1_Bahrain)
length(cov_MCV2_Bahrain)
#We assumed that NA coverage  correspond to 0 coverage
cov_MCV1_Bahrain[is.na(cov_MCV1_Bahrain)] <- 0
cov_MCV2_Bahrain[is.na(cov_MCV2_Bahrain)] <- 0
#In % 
cov_MCV1<-cov_MCV1_Bahrain/100
cov_MCV2<-cov_MCV2_Bahrain/100
cov_MCV1
cov_MCV2
...
parameters <- list(
  ...
  p = 0.047 ,                   # Probability of infection given contact
...
initP <- Bahrain_pop_in_thousands[,6]
sum(initP)
#initS <- Bahrain_pop_in_thousands[,6]
initE <-   0* Bahrain_pop_in_thousands[,6]
initI <-   0* Bahrain_pop_in_thousands[,6]
...
(cases_2000<-as.numeric(cases_allcountries_1$Value))
initI[aci] <- 1
...
sum(initI)
Pop<-Bahrain_pop_in_thousands[,6]
initR<-0 * Bahrain_pop_in_thousands[,6]
#initR[31:101] <- 0.09 * Pop[31:101]#People born before 1970 had natural immunity (no vaccine) 
#initR[1:30] <-  0.01 * Pop[1:30]   #People born after 1970 had both (natural and vaccine induced immunity):~10% of natural immunity
initV1S <- 0 * Bahrain_pop_in_thousands[,6]
initV1E <- 0 * Bahrain_pop_in_thousands[,6] # added
initV1I <- 0 * Bahrain_pop_in_thousands[,6]
initV1R<-0 * Bahrain_pop_in_thousands[,6]
#initV1R[0:30] <- 0.02* Pop[0:30]  # 2% of the population had a vaccine induced immunity( Here we assumed a low vaccination coverage before 2000)
initV2S <- 0 * Bahrain_pop_in_thousands[,6]
initV2E <- 0 * Bahrain_pop_in_thousands[,6] # added
initV2I <- 0 * Bahrain_pop_in_thousands[,6]
initV2R <- 0 * Bahrain_pop_in_thousands[,6]
#initV2R[2:30] <- 0.01* Pop[2:30] #Here we assumed a low coverage of MCV2 than MCV1
initSIAS <-0 * Bahrain_pop_in_thousands[,6]
initSIAE <-0 * Bahrain_pop_in_thousands[,6] # added
initSIAI <-0 * Bahrain_pop_in_thousands[,6]
initSIAR <-0 * Bahrain_pop_in_thousands[,6]
#Hospitalization initial conditions 
initH0 <- 0 * Bahrain_pop_in_thousands[,6]
initH1 <- 0 * Bahrain_pop_in_thousands[,6]
initH2 <- 0 * Bahrain_pop_in_thousands[,6]
initH3 <- 0 * Bahrain_pop_in_thousands[,6]
#Cumulative mortality initial conditions
initD0 <- 0 * Bahrain_pop_in_thousands[,6]
initD1 <- 0 * Bahrain_pop_in_thousands[,6]
initD2 <- 0 * Bahrain_pop_in_thousands[,6]
initD3 <- 0 * Bahrain_pop_in_thousands[,6]
initDTotal <- 0 * Bahrain_pop_in_thousands[,6]
# Wasted dose initial conditions 
initW1 <- 0 * Bahrain_pop_in_thousands[,6]      #  MCV1 doses
initW2 <- 0 * Bahrain_pop_in_thousands[,6]      #  MCV2 doses  
initW3 <- 0 * Bahrain_pop_in_thousands[,6]      #  SIA doses
initWTotal <- 0 * Bahrain_pop_in_thousands[,6]  #  Total wasted doses
initS <- Bahrain_pop_in_thousands[,6] - (initE+initI + initR + initV1S + initV1E + initV1I + initV1R + 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Model function
Bahrain_model <- function(t, Y, parameters) {
  with(as.list(c(Y, parameters)),
       {
         ...
         seas <- 1 # +amp*cos(2*pi*(t-phi/12))  # Seasonal forcing (commented out)
         
         # Check dimensions and handle division by zero
         infectious_proportion <- ifelse(N > 0, total_infectious/N, 0)
         lambda <- seas * p * (m_contact_1y %*% infectious_proportion)
...
output_Bahrain <- ode(y = Y,times = times,func = Bahrain_model,parms = parameters,method = "lsoda")
end_time <- Sys.time()# End timer
(duration <- end_time - start_time)

# I.Process results_Bahrain
results_Bahrain <- as.data.frame(output_Bahrain)
...
colnames(results_Bahrain) <- col_names
names(results_Bahrain)
...
total_pop<- rowSums(results_Bahrain[, 2:(20*n_age)])  # This will exclude time column
...
ggplot(P_t, aes(x = tim, y = total_pop)) +
  ...
  labs(
    title = "Total population over time in Bahrain",
...
#Prob.susceptible
#Susceptible
results_Bahrain$susceptible <- rowSums( 
  results_Bahrain[, (Sindex + 1)])+
  rowSums(results_Bahrain[, (V1Sindex + 1)])+
  rowSums(results_Bahrain[, (V2Sindex + 1)])+
  rowSums(results_Bahrain[, (SIASindex + 1)])
prop_sus_t<-(results_Bahrain$susceptible/total_pop)
...
ggplot(P_t, aes(x = tim, y = prop_sus_t)) +
  ...
  geom_line(color = "blue") +
  labs(
    title = "Total susceptible over time in Bahrain",
...
# 7.Reshape results_Bahrain for data analysis
names(results_Bahrain)
time_index <- which(names(results_Bahrain) == "time")
# Select "time" and next columns 20*101
20*101
selected <- results_Bahrain[, c(time_index, (time_index + 1):(time_index + 20*n_age))]
column_names <- names(selected)
results_Bahrain_0<-results_Bahrain[column_names]
#8.Long format of model results_Bahrain
#results_Bahrain<-results_Bahrain[,1:((16*n_age)+1)] #12 are SIR, 4 others are H0,H1,H2,H3
pacman::p_load(data.table) # This package will allow us to reshape dataset faster
results_Bahrain_0<- as.data.table(results_Bahrain_0)
#Long format using melt to be faster
results_Bahrain_long <- melt(
  results_Bahrain_0,
  id.vars = "time",
...
# data.table 
results_Bahrain_long <- as.data.table(results_Bahrain_long)
#
results_Bahrain_long[, c("compartment", "age_group") := tstrsplit(variable, "_", fixed = TRUE)]
results_Bahrain_long[, `:=`(
  age_group = factor(age_group, levels = age_groups),
  time_years = time / 365,
  time_days = time
)]
results_Bahrain_long[, variable := NULL]
#Structure of the long format
names(results_Bahrain_long)
head(results_Bahrain_long)
table(results_Bahrain_long$compartment)
#Here i will need to speed the visualization 
pacman::p_load(data.table,ggplot2,scales)
# Data.table 
setDT(results_Bahrain_long)
#Transformations in one data.table 
results_Bahrain_long_optimized <- results_Bahrain_long[, `:=`(
...
results_Bahrain_long_optimized[as.integer(age_group) <= 101, `:=`(
  total_by_age = sum(value), 
  proportion = value / sum(value) * 100
), by = age_group]

#Immunity column 
results_Bahrain_long_optimized[, Immunity := fcase(
...
# Levels efficiently
results_Bahrain_long_optimized[, Vaccine := factor(Vaccine, levels = c("0-Dose", "1-Dose", "2-Doses", "SIA-Dose"))]
# FUNCTIONAL IMMUNITY VISUALIZATION

# Pre-aggregate first
Final_Bahrain_immunity_summary <- results_Bahrain_long_optimized[
...
names(Final_Bahrain_immunity_summary)
dim(Final_Bahrain_immunity_summary)
immunity1 <- ggplot(Final_Bahrain_immunity_summary, 
                    aes(x = age_group, y = proportion, fill = Immunity)) +
  geom_col(position = "stack") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8)) +
  labs(title = "Functional Immunity in Bahrain", x = "Age", y = "Percent of the population") +
  scale_fill_manual(values = c("MCV1" = "#00c4aa", "MCV2" = "#e573f3", 
                               "Natural" = "#00b3f4", "SIA" = "#9b9602", "Non-immune" = "#fc726c"))
print(immunity1)
# Save
ggsave("Final_immunity_Bahrain_stacked_plot.png", plot = immunity1, width = 10, height = 10)
#Data to be used to compare countries
write.csv(Final_Bahrain_immunity_summary,"Final_Bahrain_immunity_summary.csv")

# Fast file writing
fwrite(results_Bahrain_long_optimized, "Final_FI_Bahrain.csv")
# Fast image saving
ggsave("3.U.1.Immunity_stack_Bahrain_0.png", plot = immunity1, 
       width = 10, height = 10, dpi = 300)

# Pre-aggregate data  (for dodged plot)
immunity2_summary <- results_Bahrain_long_optimized[
...
print(immunity2)
# Save
ggsave("Final_immunity_Bahrain_dodged_plot.png", plot = immunity2, width = 10, height = 10)

#
#Age
table(results_Bahrain_long$age_group)
#Compartments

#Compartments
table(results_Bahrain_long$compartment)

(results_Bahrain_long1 <- results_Bahrain_long %>%
    mutate(
      Status = case_when(
        compartment %in% c("I", "V1I", "V2I", "SIAI","H0", "H1", "H2","H3") ~ "Infectious",
        compartment %in% c("E", "V1E", "V2E", "SIAE") ~ "Exposed",
...
results_Bahrain_long1
names(results_Bahrain_long1)
head(results_Bahrain_long1)
#Vaccination status 
ggplot(results_Bahrain_long1|>
         filter(age_group %in% levels(age_group)[1:50])
       , aes(x = age_group, y = value, fill = Vaccine)) +
  geom_col(position = "stack") +  # "dodge" :for side-by-side bars, stack for stacked bars
  labs(title = "Vaccination status population in Bahrain",
       x = "Age group",
       y = "Individuals ",
       fill = "Status") +
  theme_minimal() +
...
end <- max(results_Bahrain_long1$time_days)
#
final_vaccination <- results_Bahrain_long1 %>%
...
parameters
#Checks
table(results_Bahrain_long1$Status)
table(results_Bahrain_long1$Vaccine)
#Vaccination status (number)
pacman::p_load(dplyr,ggplot2,scales)
plot_vaccine <- results_Bahrain_long1 %>%
  filter(age_group %in% levels(age_group)[1:50]) %>%
  ggplot(aes(x = age_group, y = value, fill = Vaccine)) +
  geom_col(position = "dodge") +
  labs(
    title = "Vaccination status in Bahrain",
    x = "Age Group",
...
#results_Bahrain_long1_proportion
results_Bahrain_long1_proportion <- results_Bahrain_long1 %>%
  #filter(Status == "Immune") %>%
  group_by(age_group) %>%
  mutate(
    total_by_age = sum(value),
    proportion = value / total_by_age * 100
  ) %>%
  ungroup() %>%
  # Reorder vaccine categories to match the original chart order
  mutate(
    Vaccine = factor(Vaccine, levels = c(
      "0-Dose", 
      "1-Dose", 
      "2-Doses", 
      "SIA-Dose"
...
#results_Bahrain_long_with_prop_and_combined_satutus_vaccine
results_Bahrain_long_with_prop_and_combined_satutus_vaccine <- results_Bahrain_long1 %>%
  group_by(age_group) %>%
  mutate(
    total_by_age = sum(value),
    proportion = value / total_by_age * 100
  ...
  ungroup() %>%
  mutate(
    # Combined status-vaccine categories
    Immunity = case_when(
      #Status == "Susceptible" & Vaccine == "Unvaccinated" ~ "Susceptible unvaccinated",
      Status == "Immune" & Vaccine == "0-Dose" ~ "Natural", 
      Status == "Immune" & Vaccine == "1-Dose" ~ "MCV1",
      Status == "Immune" & Vaccine == "2-Doses" ~ "MCV2",
      Status == "Immune" & Vaccine == "SIA-Dose" ~ "SIA",#,
      #TRUE ~ paste(Status, Vaccine) # With out TRUE, will have NA
      TRUE~"Non-immune"
    )
  )
#Checks
table(results_Bahrain_long_with_prop_and_combined_satutus_vaccine$Status)
table(results_Bahrain_long_with_prop_and_combined_satutus_vaccine$Vaccine)
table(results_Bahrain_long_with_prop_and_combined_satutus_vaccine$Vaccine,results_Bahrain_long_with_prop_and_combined_satutus_vaccine$Status)
# Filtering specific ages
results_Bahrain_long_with_prop_and_combined_satutus_vaccine<-results_Bahrain_long_with_prop_and_combined_satutus_vaccine|>
  filter(as.integer(age_group) <= 101)#filter(age_group %in% levels(age_group)[0:101])

#Functional immunity by age
write.csv(results_Bahrain_long_with_prop_and_combined_satutus_vaccine,"Final_FI_Bahrain.csv",row.names=FALSE)
#Visualization of FI and Immunity gaps
#Functional immunity by age
immunity1<-ggplot(results_Bahrain_long_with_prop_and_combined_satutus_vaccine,
                  aes(x = age_group, y = proportion, fill = Immunity)) +
  geom_col(position = "stack") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1,size = 2)) +
  labs(title = "Functional Immunity in Bahrain",
       x="Age",
...
#print(immunity1)
#ggsave("3.U.1.Immunity_stack_Bahrain_0.png", plot =immunity1, width = 10, height = 10, dpi = 300)
#Immunity gaps by age
immunity2<-ggplot(results_Bahrain_long_with_prop_and_combined_satutus_vaccine,
                  aes(x = age_group, y = proportion, fill = Immunity)) +
  geom_col(position = "dodge") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1,size = 2)) +
...
#print(immunity2)
#ggsave("3.U.1.Immunity_dodge_Bahrain_0.png", plot =immunity2, width = 10, height = 10, dpi = 300)
#dev.off()
#Packages for visualization
...
infectious_bar_plot1_a <- ggplot(results_Bahrain_long1 %>%
...
infectious_bar_plot1_b <- ggplot(results_Bahrain_long1 %>%
  ...
  labs(x = "Age group", y = "Number Infectious",
       title = "Measles cases and vaccination status in Bahrain") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
calculate_effective_Rt_parallel <- function(results_Bahrain, parameters) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Bahrain[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Bahrain$time,
    time_years = results_Bahrain$time / 365,
    R_eff = unlist(R_eff)
  )
}
start_time <- Sys.time()
Rt_data_Bahrain <- calculate_effective_Rt_parallel(results_Bahrain, parameters)
end_time <- Sys.time()
end_time - start_time


# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Bahrain, aes(x = time_years, y = R_eff)) +
    ...
    annotate("text", x = 2, y = max(Rt_data_Bahrain$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Bahrain$R_eff) * 0.8, 
...
(max_Reff<-max(Rt_data_Bahrain$R_eff))
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Bahrain$year <- 2000 + Rt_data_Bahrain$time_years
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Bahrain, aes(x = time_years, y = R_eff)) +
    ...
    geom_point(data = Rt_data_Bahrain[seq(1, nrow(Rt_data_Bahrain), by = 50), ], 
               color = "#E63946", size = 1) +
    ...
    # Text annotations
    annotate("text", x = max(Rt_data_Bahrain$time_years) * 0.8, y = max(Rt_data_Bahrain$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Bahrain$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Bahrain$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    
    labs(
      x = "Time (Years)",
      y = "Rt",
      title = "Epidemic control assessment in Bahrain :Baseline",
      #subtitle = "Green:disease control (Rt < 1)|Red :Potential epidemic spread (Rt > 1)"
    ) +
    scale_y_continuous(limits = c(0, max(Rt_data_Bahrain$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
#Year in thousand
Rt_data_Bahrain$year <- as.numeric(2000 + Rt_data_Bahrain$time_years)

Rt_zone_plot <- ggplot(Rt_data_Bahrain, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Bahrain$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
  ...
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Bahrain$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Bahrain$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Bahrain$R_eff) * 0.1, 
...
(Rt_hist_plot <- ggplot(Rt_data_Bahrain, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Bahrain$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Bahrain$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Bahrain$R_eff) + 0.1, y = max(table(cut(Rt_data_Bahrain$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Bahrain$R_eff), 2)), 
    ...
    labs(
      ...
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Bahrain$R_eff < 1) / nrow(Rt_data_Bahrain), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
vaccine_coverage_over_time <- results_Bahrain_long1 %>%
...
(Rt_coverage_plot <- ggplot(combined_data, aes(x = time_years)) +
    ...
    labs(
      x = "Time (Years)",
      title = "Rt and vaccination coverage over time in Bahrain"
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
start_time <- Sys.time()
output_Bahrain_scenario_1 <- ode(y = Y,times = times,func = Bahrain_model,parms = parameters_1,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Bahrain
results_Bahrain_scenario_1 <- as.data.frame(output_Bahrain_scenario_1)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Bahrain_scenario_1) <- col_names
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
calculate_effective_Rt_parallel <- function(results_Bahrain_scenario_1, parameters_1) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Bahrain_scenario_1[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Bahrain_scenario_1$time,
    time_years = results_Bahrain_scenario_1$time / 365,
    R_eff = unlist(R_eff)
  )
}
...
Rt_data_Bahrain_scenario_1 <- calculate_effective_Rt_parallel(results_Bahrain_scenario_1, parameters_1)
end_time <- Sys.time() #End
end_time - start_time  #Duration
# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_1, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Bahrain_scenario_1$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Bahrain_scenario_1$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
#
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Bahrain_scenario_1$year <- 2000 + Rt_data_Bahrain_scenario_1$time_years

# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_1, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Bahrain_scenario_1[seq(1, nrow(Rt_data_Bahrain_scenario_1), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Bahrain_scenario_1$time_years) * 0.8, y = max(Rt_data_Bahrain_scenario_1$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 2, fontface = "bold") +
    annotate("text", x = max(Rt_data_Bahrain_scenario_1$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 2, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Bahrain_scenario_1$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    
    labs(
      x = "Time (Years)",
      y = "Rt",
      title = "Epidemic control assessment in Bahrain :SIA-RI",
      #subtitle = "Green:disease control (Rt < 1)|Red :Potential epidemic spread (Rt > 1)"
    ) +
    scale_y_continuous(limits = c(0, max(Rt_data_Bahrain_scenario_1$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Bahrain_scenario_1$year <- as.numeric(2000 + Rt_data_Bahrain_scenario_1$time_years)

Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_1, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Bahrain_scenario_1$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Bahrain_scenario_1[seq(1, nrow(Rt_data_Bahrain_scenario_1), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Bahrain_scenario_1$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Bahrain_scenario_1$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Bahrain_scenario_1$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
Rt_zone_plot
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Bahrain_scenario_1, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Bahrain_scenario_1$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Bahrain_scenario_1$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Bahrain_scenario_1$R_eff) + 0.1, y = max(table(cut(Rt_data_Bahrain$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Bahrain_scenario_1$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      ...
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Bahrain_scenario_1$R_eff < 1) / nrow(Rt_data_Bahrain_scenario_1), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
end_time <- Sys.time()
output_Bahrain_scenario_2 <- ode(y = Y,times = times,func = Bahrain_model,parms = parameters_2,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Bahrain
results_Bahrain_scenario_2 <- as.data.frame(output_Bahrain_scenario_2)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Bahrain_scenario_2) <- col_names
#Rt-2
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Epidemic control assessment  using Rt 
...
calculate_effective_Rt_parallel <- function(results_Bahrain_scenario_2, parameters_2) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Bahrain_scenario_2[, idx(offset)])
  }
  ...
  # Result
  data.frame(
    time_days = results_Bahrain_scenario_2$time,
    time_years = results_Bahrain_scenario_2$time / 365,
    R_eff = unlist(R_eff)
  )
}
...
#  Rt data
start_time <- Sys.time()
Rt_data_Bahrain_scenario_2 <- calculate_effective_Rt_parallel(results_Bahrain_scenario_2,parameters_2)
end_time <- Sys.time()
end_time - start_time

# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_2, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
#
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Bahrain_scenario_2$year <- 2000 + Rt_data_Bahrain_scenario_2$time_years

# Plot using calendar years on x-axis
Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_2, aes(x = year, y = R_eff)) +
  geom_line(color = "#E63946", size = 1.2) +
  ...
  # Annotations
  annotate("text", x = 2005, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.9, 
           label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
  annotate("text", x = 2005, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.8, 
           label = "SIA campaigns", color = "blue", size = 3) +
...
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_2, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Bahrain_scenario_2[seq(1, nrow(Rt_data_Bahrain_scenario_2), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Bahrain_scenario_2$time_years) * 0.8, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Bahrain_scenario_2$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    ...
    scale_y_continuous(limits = c(0, max(Rt_data_Bahrain_scenario_2$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Bahrain_scenario_2$year <- as.numeric(2000 + Rt_data_Bahrain_scenario_2$time_years)

Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_2, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Bahrain_scenario_2$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Bahrain_scenario_2[seq(1, nrow(Rt_data_Bahrain_scenario_2), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Bahrain_scenario_2$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
#plotly::ggplotly(Rt_zone_plot)
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Bahrain_scenario_2, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Bahrain_scenario_2$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Bahrain_scenario_2$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Bahrain_scenario_2$R_eff) + 0.1, y = max(table(cut(Rt_data_Bahrain_scenario_2$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Bahrain_scenario_2$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      ...
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Bahrain_scenario_2$R_eff < 1) / nrow(Rt_data_Bahrain_scenario_2), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
names(vaccine_coverage_over_time)
# Rt and vaccination data
combined_data <- Rt_data_Bahrain_scenario_2 %>%
  left_join(vaccine_coverage_over_time, by = c("time_days", "time_years"))
...
output_Bahrain_scenario_3 <- ode(y = Y,times = times,func = Bahrain_model,parms = parameters_3,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Bahrain
results_Bahrain_scenario_3 <- as.data.frame(output_Bahrain_scenario_3)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Bahrain_scenario_3) <- col_names
...
calculate_effective_Rt_parallel <- function(results_Bahrain_scenario_3, parameters_3) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Bahrain_scenario_3[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Bahrain_scenario_3$time,
    time_years = results_Bahrain_scenario_3$time / 365,
    R_eff = unlist(R_eff)
  )
}
...
#  Rt data
start_time<-Sys.time()
(Rt_data_Bahrain_scenario_3 <- calculate_effective_Rt_parallel(results_Bahrain_scenario_3, parameters_3))
end_time<-Sys.time()
(end_time-start_time)

# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_3, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
#
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Bahrain_scenario_3$year <- 2000 + Rt_data_Bahrain_scenario_3$time_years
dim(Rt_data_Bahrain_scenario_3)
# Plot using calendar years on x-axis
Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_3, aes(x = year, y = R_eff)) +
  geom_line(color = "#E63946", size = 1.2) +
  ...
  # Annotations
  annotate("text", x = 2005, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.9, 
           label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
  annotate("text", x = 2005, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.8, 
           label = "SIA campaigns", color = "blue", size = 3) +
...
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_3, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Bahrain_scenario_3[seq(1, nrow(Rt_data_Bahrain_scenario_3), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Bahrain_scenario_3$time_years) * 0.8, y = max(Rt_data_Bahrain_scenario_2$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Bahrain_scenario_3$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    ...
    scale_y_continuous(limits = c(0, max(Rt_data_Bahrain_scenario_3$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Bahrain_scenario_3$year <- as.numeric(2000 + Rt_data_Bahrain_scenario_3$time_years)

Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_3, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Bahrain_scenario_3$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Bahrain_scenario_3[seq(1, nrow(Rt_data_Bahrain_scenario_3), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Bahrain_scenario_3$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Bahrain_scenario_3$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
Rt_zone_plot
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Bahrain_scenario_3, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Bahrain_scenario_3$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Bahrain_scenario_3$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Bahrain_scenario_3$R_eff) + 0.1, y = max(table(cut(Rt_data_Bahrain_scenario_3$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Bahrain_scenario_3$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      title = "Distribution of Rt values",
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Bahrain_scenario_3$R_eff < 1) / nrow(Rt_data_Bahrain_scenario_3), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
names(vaccine_coverage_over_time)
# Rt and vaccination data
combined_data <- Rt_data_Bahrain_scenario_3 %>%
  left_join(vaccine_coverage_over_time, by = c("time_days", "time_years"))
...
parameters_4$rate1<--log(1-parameters_4$cov_MCV1)/parameters_4$sia_duration
parameters_4$rate2<--log(1-parameters_4$cov_MCV2)/parameters_4$sia_duration
#Timer
end_time <- Sys.time()
output_Bahrain_scenario_4 <- ode(y = Y,times = times,func = Bahrain_model,parms = parameters_4,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Bahrain
results_Bahrain_scenario_4 <- as.data.frame(output_Bahrain_scenario_4)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Bahrain_scenario_4) <- col_names
...
calculate_effective_Rt_parallel <- function(results_Bahrain_scenario_4, parameters_4) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Bahrain_scenario_4[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Bahrain_scenario_4$time,
    time_years = results_Bahrain_scenario_4$time / 365,
    R_eff = unlist(R_eff)
  )
}
# Rt data
start_time<-Sys.time()
(Rt_data_Bahrain_scenario_4 <- calculate_effective_Rt_parallel(results_Bahrain_scenario_4, parameters_4))
end_time<-Sys.time()
(end_time-start_time)

# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_4, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Bahrain_scenario_4$year <- 2000 + Rt_data_Bahrain_scenario_4$time_years

# Plot using calendar years on x-axis
Rt_basic_plot <- ggplot(Rt_data_Bahrain_scenario_4, aes(x = year, y = R_eff)) +
  geom_line(color = "#E63946", size = 1.2) +
  ...
  # Annotations
  annotate("text", x = 2005, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.9, 
           label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
  annotate("text", x = 2005, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.8, 
           label = "SIA campaigns", color = "blue", size = 3) +
...
Rt_basic_plot
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_4, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Bahrain_scenario_4[seq(1, nrow(Rt_data_Bahrain_scenario_4), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Bahrain_scenario_4$time_years) * 0.8, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Bahrain_scenario_4$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    ...
    scale_y_continuous(limits = c(0, max(Rt_data_Bahrain_scenario_4$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Bahrain_scenario_4$year <- as.numeric(2000 + Rt_data_Bahrain_scenario_4$time_years)

Rt_zone_plot <- ggplot(Rt_data_Bahrain_scenario_4, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Bahrain_scenario_4$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Bahrain_scenario_4[seq(1, nrow(Rt_data_Bahrain_scenario_4), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Bahrain_scenario_4$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Bahrain_scenario_4$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
#plotly::ggplotly(Rt_zone_plot)
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Bahrain_scenario_4, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Bahrain_scenario_4$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Bahrain_scenario_4$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Bahrain_scenario_4$R_eff) + 0.1, y = max(table(cut(Rt_data_Bahrain_scenario_4$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Bahrain_scenario_4$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      title = "Distribution of Rt values",
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Bahrain_scenario_4$R_eff < 1) / nrow(Rt_data_Bahrain_scenario_4), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
names(vaccine_coverage_over_time)
# Rt and vaccination data
combined_data <- Rt_data_Bahrain_scenario_4 %>%
  left_join(vaccine_coverage_over_time, by = c("time_days", "time_years"))
...
parameters_5$eff3_c <- 1e-6 #No vaccine in individual with natural immunity induced immunity immunity
# Timer
start_time <- Sys.time()
output_Bahrain_scenario_5 <- ode(y = Y, times = times,func = Bahrain_model,parms = parameters_5,method = "vode")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results
results_Bahrain_scenario_5 <- as.data.frame(output_Bahrain_scenario_5)
# Rename columns
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Bahrain_scenario_5) <- col_names
...
#0.Baseline
#~~~~~~~~~~~~~~~~~~~~~~~>
results_Bahrain<-as.data.frame(results_Bahrain)
results_Bahrain$scenario<-"Baseline"
results_Bahrain$Country<-"Bahrain"
#0.a.Incidence
results_Bahrain$incidence <- rowSums( results_Bahrain[, (Iindex + 1)])+
  rowSums( results_Bahrain[, (V1Iindex +  1)])+
  rowSums( results_Bahrain[, (V2Iindex +  1)])+
  rowSums( results_Bahrain[, (SIAIindex+  1)])+
  rowSums( results_Bahrain[, (H0index  +  1)])+
  rowSums( results_Bahrain[, (H1index  +  1)])+
  rowSums( results_Bahrain[, (H2index  +  1)])+
  rowSums( results_Bahrain[, (H3index  +  1)])
#0.b.cum incidence
results_Bahrain$cum_incidence<-cumsum(results_Bahrain$incidence)
#0.c.Hospitalization
results_Bahrain$hospitalization <-rowSums( results_Bahrain[, (H0index  +  1)])+
  rowSums( results_Bahrain[, (H1index  +  1)])+
  rowSums( results_Bahrain[, (H2index  +  1)])+
  rowSums( results_Bahrain[, (H3index  +  1)])
#0.d.cum_hospitalization
results_Bahrain$cum_hospitalization<-cumsum(results_Bahrain$hospitalization)
head(results_Bahrain)
dim(results_Bahrain)
#0.e. deaths
results_Bahrain$deaths <-rowSums( results_Bahrain[, (DTotalindex  +  1)])
#0.f.cum_deaths
results_Bahrain$cum_deaths<-cumsum(results_Bahrain$deaths)
#0.g.wasted_doses
results_Bahrain$wasted_doses <-rowSums( results_Bahrain[, (WTotalindex  +  1)])
#0.h.cum_wasted_doses
results_Bahrain$cum_wasted_doses<-cumsum(results_Bahrain$wasted_doses)

#1.Scenario_1:RI-SIA-2Y
#~~~~~~~~~~~~~~~~~~~~~~>
results_Bahrain_scenario_1<-as.data.frame(results_Bahrain_scenario_1)
results_Bahrain_scenario_1$scenario<-"RI-SIA-2Y"
results_Bahrain_scenario_1$Country<-"Bahrain"
# 1.a. Incidence
results_Bahrain_scenario_1$incidence <- rowSums(results_Bahrain_scenario_1[, (Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (V1Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (V2Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (SIAIindex + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H3index + 1)])

# 1.b. Cumulative incidence
results_Bahrain_scenario_1$cum_incidence <- cumsum(results_Bahrain_scenario_1$incidence)

# 1.c. Hospitalization
results_Bahrain_scenario_1$hospitalization <- rowSums(results_Bahrain_scenario_1[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_1[, (H3index + 1)])

# 1.d. Cumulative hospitalization
results_Bahrain_scenario_1$cum_hospitalization <- cumsum(results_Bahrain_scenario_1$hospitalization)

# 1.e. Deaths
results_Bahrain_scenario_1$deaths <- rowSums(results_Bahrain_scenario_1[, (DTotalindex + 1)])

# 1.f. Cumulative deaths
results_Bahrain_scenario_1$cum_deaths <- cumsum(results_Bahrain_scenario_1$deaths)

# 1.g. Wasted doses
results_Bahrain_scenario_1$wasted_doses <- rowSums(results_Bahrain_scenario_1[, (WTotalindex + 1)])

# 1.h. Cumulative wasted doses
results_Bahrain_scenario_1$cum_wasted_doses <- cumsum(results_Bahrain_scenario_1$wasted_doses)

head(results_Bahrain_scenario_1)
dim(results_Bahrain_scenario_1)

#Scenario-2: "RI-scaled-up"
#~~~~~~~~~~~~~~~~~~~~~~~~~>
results_Bahrain_scenario_2<-as.data.frame(results_Bahrain_scenario_2)
results_Bahrain_scenario_2$scenario<-"RI-scaled-up"
results_Bahrain_scenario_2$Country<-"Bahrain"
# 2.a. Incidence
results_Bahrain_scenario_2$incidence <- rowSums(results_Bahrain_scenario_2[, (Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (V1Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (V2Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (SIAIindex + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H3index + 1)])

# 2.b. Cumulative incidence
results_Bahrain_scenario_2$cum_incidence <- cumsum(results_Bahrain_scenario_2$incidence)

# 2.c. Hospitalization
results_Bahrain_scenario_2$hospitalization <- rowSums(results_Bahrain_scenario_2[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_2[, (H3index + 1)])

# 2.d. Cumulative hospitalization
results_Bahrain_scenario_2$cum_hospitalization <- cumsum(results_Bahrain_scenario_2$hospitalization)

# 2.e. Deaths
results_Bahrain_scenario_2$deaths <- rowSums(results_Bahrain_scenario_2[, (DTotalindex + 1)])

# 2.f. Cumulative deaths
results_Bahrain_scenario_2$cum_deaths <- cumsum(results_Bahrain_scenario_2$deaths)

# 2.g. Wasted doses
results_Bahrain_scenario_2$wasted_doses <- rowSums(results_Bahrain_scenario_2[, (WTotalindex + 1)])

# 2.h. Cumulative wasted doses
results_Bahrain_scenario_2$cum_wasted_doses <- cumsum(results_Bahrain_scenario_2$wasted_doses)
#checks
head(results_Bahrain_scenario_2)
dim(results_Bahrain_scenario_2)
#Scenario_3:"RI-SIA-5Y"
#~~~~~~~~~~~~~~~~~~~~~>
results_Bahrain_scenario_3<-as.data.frame(results_Bahrain_scenario_3)
results_Bahrain_scenario_3$scenario<-"RI-SIA-5Y"
results_Bahrain_scenario_3$Country<-"Bahrain"
# 3.a. Incidence
results_Bahrain_scenario_3$incidence <- rowSums(results_Bahrain_scenario_3[, (Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (V1Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (V2Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (SIAIindex + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H3index + 1)])

# 3.b. Cumulative incidence
results_Bahrain_scenario_3$cum_incidence <- cumsum(results_Bahrain_scenario_3$incidence)

# 3.c. Hospitalization
results_Bahrain_scenario_3$hospitalization <- rowSums(results_Bahrain_scenario_3[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_3[, (H3index + 1)])

# 3.d. Cumulative hospitalization
results_Bahrain_scenario_3$cum_hospitalization <- cumsum(results_Bahrain_scenario_3$hospitalization)

# 3.e. Deaths
results_Bahrain_scenario_3$deaths <- rowSums(results_Bahrain_scenario_3[, (DTotalindex + 1)])

# 3.f. Cumulative deaths
results_Bahrain_scenario_3$cum_deaths <- cumsum(results_Bahrain_scenario_3$deaths)

# 3.g. Wasted doses
results_Bahrain_scenario_3$wasted_doses <- rowSums(results_Bahrain_scenario_3[, (WTotalindex + 1)])

# 3.h. Cumulative wasted doses
results_Bahrain_scenario_3$cum_wasted_doses <- cumsum(results_Bahrain_scenario_3$wasted_doses)

# Checks
head(results_Bahrain_scenario_3)
dim(results_Bahrain_scenario_3)
#Scenario_4:"RI-Reduced"
#~~~~~~~~~~~~~~~~~~~~~~>
results_Bahrain_scenario_4<-as.data.frame(results_Bahrain_scenario_4)
results_Bahrain_scenario_4$scenario<-"RI-Reduced"
results_Bahrain_scenario_4$Country<-"Bahrain" 
# 4.a. Incidence
results_Bahrain_scenario_4$incidence <- rowSums(results_Bahrain_scenario_4[, (Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (V1Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (V2Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (SIAIindex + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H3index + 1)])

# 4.b. Cumulative incidence
results_Bahrain_scenario_4$cum_incidence <- cumsum(results_Bahrain_scenario_4$incidence)

# 4.c. Hospitalization
results_Bahrain_scenario_4$hospitalization <- rowSums(results_Bahrain_scenario_4[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_4[, (H3index + 1)])

# 4.d. Cumulative hospitalization
results_Bahrain_scenario_4$cum_hospitalization <- cumsum(results_Bahrain_scenario_4$hospitalization)

# 4.e. Deaths
results_Bahrain_scenario_4$deaths <- rowSums(results_Bahrain_scenario_4[, (DTotalindex + 1)])

# 4.f. Cumulative deaths
results_Bahrain_scenario_4$cum_deaths <- cumsum(results_Bahrain_scenario_4$deaths)

# 4.g. Wasted doses
results_Bahrain_scenario_4$wasted_doses <- rowSums(results_Bahrain_scenario_4[, (WTotalindex + 1)])

# 4.h. Cumulative wasted doses
results_Bahrain_scenario_4$cum_wasted_doses <- cumsum(results_Bahrain_scenario_4$wasted_doses)
...
dim(results_Bahrain_scenario_2)
dim(results_Bahrain_scenario_3)
dim(results_Bahrain_scenario_4)

# Impact of interventions/scnarios 
# Incidence
(scenario_1_cum_incidence <- (tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_1$cum_incidence, 1)) / tail(results_Bahrain$cum_incidence, 1) * 100)
(scenario_2_cum_incidence <- (tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_2$cum_incidence, 1)) / tail(results_Bahrain$cum_incidence, 1) * 100)
(scenario_3_cum_incidence <- (tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_3$cum_incidence, 1)) / tail(results_Bahrain$cum_incidence, 1) * 100)
(scenario_4_cum_incidence <- (tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_4$cum_incidence, 1)) / tail(results_Bahrain$cum_incidence, 1) * 100)

#Severe cases 
(scenario_1_cum_hospitalization <- (tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_1$cum_hospitalization, 1)) / tail(results_Bahrain$cum_hospitalization, 1) * 100)
(scenario_2_cum_hospitalization <- (tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_2$cum_hospitalization, 1)) / tail(results_Bahrain$cum_hospitalization, 1) * 100)
(scenario_3_cum_hospitalization <- (tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_3$cum_hospitalization, 1)) / tail(results_Bahrain$cum_hospitalization, 1) * 100)
(scenario_4_cum_hospitalization <- (tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_4$cum_hospitalization, 1)) / tail(results_Bahrain$cum_hospitalization, 1) * 100)
#Deaths
(scenario_1_cum_deaths <- (tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_1$cum_deaths, 1)) / tail(results_Bahrain$cum_deaths, 1) * 100)
(scenario_2_cum_deaths <- (tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_2$cum_deaths, 1)) / tail(results_Bahrain$cum_deaths, 1) * 100)
(scenario_3_cum_deaths <- (tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_4$cum_deaths, 1)) / tail(results_Bahrain$cum_deaths, 1) * 100)
(scenario_4_cum_deaths <- (tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_2$cum_deaths, 1)) / tail(results_Bahrain$cum_deaths, 1) * 100)


#Visualization
library(ggplot2)
#Averted cases 
averted_cases_Bahrain <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SCALED-UP", "RI-SIA-5Y", "RI-REDUCED"),
  Averted_Cases = c(
    tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_1$cum_incidence, 1),
    tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_2$cum_incidence, 1),
    tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_3$cum_incidence, 1),
    tail(results_Bahrain$cum_incidence, 1) - tail(results_Bahrain_scenario_4$cum_incidence, 1)
  )
)
...
ggplot(averted_cases_Bahrain, aes(x = Scenario, y = Averted_Cases, fill = Scenario)) +
...
write.csv(averted_cases_Bahrain,"Final_averted_cases_Bahrain.csv",row.names = FALSE)

#Severe cases
averted_severe_cases_Bahrain <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SCALED-UP", "RI-SIA-5Y", "RI-REDUCED"),
  Averted_Severe_cases = c(
    tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_1$cum_hospitalization, 1),
    tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_2$cum_hospitalization, 1),
    tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_3$cum_hospitalization, 1),
    tail(results_Bahrain$cum_hospitalization, 1) - tail(results_Bahrain_scenario_4$cum_hospitalization, 1)
  )
)

library(ggplot2)
ggplot(averted_severe_cases_Bahrain, aes(x = Scenario, y = Averted_Severe_cases, fill = Scenario)) +
  geom_bar(stat = "identity") +
  scale_fill_viridis_d() + 
  scale_y_continuous(labels = scales::label_number(scale = 1e-3, accuracy = 1, suffix = "K")) +  
  labs(title = " ",
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1))
#Save final averted deaths
write.csv(averted_severe_cases_Bahrain,"Final_averted_severe_cases_Bahrain.csv",row.names = FALSE)
#~~~~~~~~~~~~~~~~~#
# Averted deaths  #
#~~~~~~~~~~~~~~~~~#
averted_deaths_Bahrain <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SCALED-UP", "RI-SIA-5Y", "RI-REDUCED"),
  Averted_deaths = c(
    tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_1$cum_deaths, 1),
    tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_2$cum_deaths, 1),
    tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_3$cum_deaths, 1),
    tail(results_Bahrain$cum_deaths, 1) - tail(results_Bahrain_scenario_4$cum_deaths, 1)
  )
)
# Plot of averted deaths
library(ggplot2)
ggplot(averted_deaths_Bahrain, aes(x = Scenario, y = Averted_deaths, fill = Scenario)) +
  geom_bar(stat = "identity") +
  scale_fill_viridis_d() + 
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1))
#Save final averted deaths
write.csv(averted_deaths_Bahrain,"Final_averted_deaths_Bahrain.csv",row.names = FALSE)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Scenario_5: Pre-screened vaccination
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~>

# 5.a. Incidence
results_Bahrain_scenario_5$incidence <- rowSums(results_Bahrain_scenario_5[, (Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (V1Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (V2Iindex + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (SIAIindex + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H3index + 1)])

# 5.b. Cumulative incidence
results_Bahrain_scenario_5$cum_incidence <- cumsum(results_Bahrain_scenario_5$incidence)

# 5.c. Hospitalization
results_Bahrain_scenario_5$hospitalization <- rowSums(results_Bahrain_scenario_5[, (H0index + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H1index + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H2index + 1)]) +
  rowSums(results_Bahrain_scenario_5[, (H3index + 1)])

# 5.d. Cumulative hospitalization
results_Bahrain_scenario_5$cum_hospitalization <- cumsum(results_Bahrain_scenario_5$hospitalization)

# 5.e. Deaths
results_Bahrain_scenario_5$deaths <- rowSums(results_Bahrain_scenario_5[, (DTotalindex + 1)])

# 5.f. Cumulative deaths
results_Bahrain_scenario_5$cum_deaths <- cumsum(results_Bahrain_scenario_5$deaths)

# 5.g. Wasted doses
results_Bahrain_scenario_5$wasted_doses <- rowSums(results_Bahrain_scenario_5[, (WTotalindex + 1)])

# 5.h. Cumulative wasted doses
results_Bahrain_scenario_5$cum_wasted_doses <- cumsum(results_Bahrain_scenario_5$wasted_doses)

# Checks
head(results_Bahrain_scenario_5)
dim(results_Bahrain_scenario_5)

#results_Bahrain_scenarios<-rbind(results_Bahrain,
#                                     results_Bahrain_scenario_1,
#                                     results_Bahrain_scenario_2,
#                                     results_Bahrain_scenario_3,
#                                     results_Bahrain_scenario_4
#) 
#write.csv(results_Bahrain_scenarios,"0.results_Bahrain_scenarios.csv",row.names = FALSE)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# A. EPIDEMIC CONTROL ASSESSMENT: Rt data visualization
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Visualization of Rt-data
Rt_data_Bahrain$scenario<-"Baseline"
dim(Rt_data_Bahrain)
Rt_data_Bahrain_scenario_1$scenario<-"RI-SIA-2Y"
dim(Rt_data_Bahrain_scenario_1)
Rt_data_Bahrain_scenario_2$scenario<-"RI-scaled-up"
dim(Rt_data_Bahrain_scenario_2)
Rt_data_Bahrain_scenario_3$scenario<-"RI-SIA-5Y"
dim(Rt_data_Bahrain_scenario_3)
Rt_data_Bahrain_scenario_4$scenario<-"RI-Reduced"
dim(Rt_data_Bahrain_scenario_4)
Rt_data_Bahrain_scenarios<-rbind(Rt_data_Bahrain,
                                 Rt_data_Bahrain_scenario_1,
                                 Rt_data_Bahrain_scenario_2,
                                 Rt_data_Bahrain_scenario_3,
                                 Rt_data_Bahrain_scenario_4
)


#Year in thousand
Rt_data_Bahrain_scenarios$year <- as.numeric(2000 + Rt_data_Bahrain_scenarios$time_years)
write.csv(Rt_data_Bahrain_scenarios,"Final_Rt_data_Bahrain_scenarios.csv",row.names = FALSE)
#Read
Rt_data_Bahrain_scenarios <- read.csv("Final_Rt_data_Bahrain_scenarios.csv")
max_Reff <- max(Rt_data_Bahrain_scenarios$Reff, na.rm = TRUE)
#Visualization
library(ggplot2)
Final_Rt_Bahrain<-ggplot(Rt_data_Bahrain_scenarios, aes(x = year, y = R_eff, color = scenario)) +
  ...
  labs(
    title = expression("Measles time-varying " ~ R[t] ~ "in Bahrain"),
    subtitle = "Bahrain - Scenario Analysis",
    x = "Year",
    y = expression(R[eff]),
    color = "Scenario"
  ) +
...
ggsave("Final_Rt_Bahrain.png",plot=Final_Rt_Bahrain,height = 10, width = 12,dpi = 200 )
#At the end of each code. I saved the workenvironment in order to be re-used without re-running
#save.image(file = "Final_2_Bahrain.RData")


# Sensitivity Analysis for Bahrain Measles Model
...
run_sensitivity <- function(param_name, param_value) {
  ...
  # Run the model
  results <- ode(y = Y, times = times, func = Bahrain_model, parms = new_params, method = "lsoda")
...
print(p11)
# Save the plot
ggsave("sensitivity_analysis_measles_Bahrain.png",plot=p11,width=12, height=10,dpi=300)
