...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#                             SEIR Model for Measles for Palestine
...
#dev.off()
# Packages
#save.image(file = "Final_1_Palestine.RData")-->This is the saved enviroment to be more fast
#load("Final_1_Palestine.RData")
...
#1.b.Population structure (Palestine) / 1 year age group
Population_emro_2023 <- read_csv("Population_emro_2023_1yearage.csv")
table(Population_emro_2023$Country)
(Palestine_pop<-as.data.frame(Population_emro_2023|>
                               filter( Year=="2023")|>
...
(Palestine_pop_in_thousands<-as.data.frame(Palestine_pop|>
                                            mutate(Population_age=Population_age*1000)))
#1.c. Visualization of Palestine's population structure
pacman::p_load(ggplot2,dplyr,plotly)
names(Palestine_pop_in_thousands)
(pyramyd<-ggplot(Palestine_pop_in_thousands, aes(x = Age_Category, y = Population_age)) +
    geom_col(fill = "steelblue") +
    labs(title = "Palestine's population structure (2023)",
         x = "Age", y = "Population") +
    theme_minimal() +
...
#ggplotly(pyramyd)
# Shorten  the name
popstruc<-Palestine_pop_in_thousands
#1.e.Number of age groups 
(A<-n_age<-length(Palestine_pop_in_thousands[,6]))
...
(pyramyd<-ggplot(popbirth, aes(x = Age, y = Birth)) +
    geom_col(fill = "blue") +
    labs(title = "Birth in Palestine (2023)",
         x = "Age", y = "Births") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
)
...
(popmort<-as.data.frame(mortality|>filter(Country=="State of Palestine")|>
                          filter(Year==2023)))
(pyramyd<-ggplot(popmort , aes(x =Age , y = Percentage)) +
    geom_col(fill = "red") +
    labs(title = "Mortality in Palestine (2023)",
...
(m_contact_1y <- as.matrix(read.csv("3.U.1.contact_Palestine_1y.csv")))
dim(m_contact_1y)
...
(p<-ggplot(df, aes(x = Contactor, y = Contactee, fill = Contacts)) +
    ...
    theme_minimal() +
    labs(title = "Contact Matrix Heatmap for Palestine")
)
#(p_plotly <- ggplotly(p))
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#           1.i. Aging
...
#Let me select Palestine and WHO/UNICEF estimated coverage
table(coverage$NAME)
coverage<-coverage|>
  filter(COVERAGE_CATEGORY=="WUENIC")|>
  filter(NAME=="occupied Palestinian territory, including east Jerusalem")
#I will need to select antigens 
MCV1_Palestine<-coverage|>
  filter(ANTIGEN=="MCV1")
MCV2_Palestine<-coverage|>
  filter(ANTIGEN=="MCV2")
print(MCV2_Palestine)
table(MCV2_Palestine$YEAR)
#I will need the coverage only  
#Secondly,  i will reverse the order in order to have 2000-2023 instead 2023-2000 
cov_MCV1_Palestine<-rev(MCV1_Palestine[,11])
cov_MCV2_Palestine<-rev(MCV2_Palestine[,11])
cov_MCV1_Palestine
cov_MCV2_Palestine
length(cov_MCV1_Palestine)
length(cov_MCV2_Palestine)
#We assumed that NA coverage  correspond to 0 coverage
cov_MCV1_Palestine[is.na(cov_MCV1_Palestine)] <- 0
cov_MCV2_Palestine[is.na(cov_MCV2_Palestine)] <- 0
#In % 
cov_MCV1<-cov_MCV1_Palestine/100
cov_MCV2<-cov_MCV2_Palestine/100
cov_MCV1
cov_MCV2
...
initP <- Palestine_pop_in_thousands[,6]
sum(initP)
#initS <- Palestine_pop_in_thousands[,6]
initE <-   0* Palestine_pop_in_thousands[,6]
initI <-   0* Palestine_pop_in_thousands[,6]
...
(cases_allcountries_1<-cases_allcountries_1|>
    filter(Location=="occupied Palestinian territory, including east Jerusalem")|>
...
initI[aci] <- 10
sum(initI)
Pop<-Palestine_pop_in_thousands[,6]
initR<-0 * Palestine_pop_in_thousands[,6]
...
initV1S <- 0 * Palestine_pop_in_thousands[,6]
initV1E <- 0 * Palestine_pop_in_thousands[,6] # added here for my SEIR structure
initV1I <- 0 * Palestine_pop_in_thousands[,6]
initV1R<-0 * Palestine_pop_in_thousands[,6]
...
initV2S <- 0 * Palestine_pop_in_thousands[,6]
initV2E <- 0 * Palestine_pop_in_thousands[,6] # added
initV2I <- 0 * Palestine_pop_in_thousands[,6]
initV2R <- 0 * Palestine_pop_in_thousands[,6]
initV2R[2:30] <- 0.01* Pop[2:30] #Here we assumed a low coverage of MCV2 than MCV1
initSIAS <-0 * Palestine_pop_in_thousands[,6]
initSIAE <-0 * Palestine_pop_in_thousands[,6] # added
initSIAI <-0 * Palestine_pop_in_thousands[,6]
initSIAR <-0 * Palestine_pop_in_thousands[,6]
#Hospitalization initial conditions 
initH0 <- 0 * Palestine_pop_in_thousands[,6]
initH1 <- 0 * Palestine_pop_in_thousands[,6]
initH2 <- 0 * Palestine_pop_in_thousands[,6]
initH3 <- 0 * Palestine_pop_in_thousands[,6]
#Cumulative mortality initial conditions
initD0 <- 0 * Palestine_pop_in_thousands[,6]
initD1 <- 0 * Palestine_pop_in_thousands[,6]
initD2 <- 0 * Palestine_pop_in_thousands[,6]
initD3 <- 0 * Palestine_pop_in_thousands[,6]
initDTotal <- 0 * Palestine_pop_in_thousands[,6]
# Wasted dose initial conditions 
initW1 <- 0 * Palestine_pop_in_thousands[,6]      #  MCV1 doses
initW2 <- 0 * Palestine_pop_in_thousands[,6]      #  MCV2 doses  
initW3 <- 0 * Palestine_pop_in_thousands[,6]      #  SIA doses
initWTotal <- 0 * Palestine_pop_in_thousands[,6]  #  Total wasted doses
initS <- Palestine_pop_in_thousands[,6] - (initE+initI + initR + initV1S + initV1E + initV1I + initV1R + 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Model function
Palestine_model <- function(t, Y, parameters) {
...
output_Palestine <- ode(y = Y,times = times,func = Palestine_model,parms = parameters,method = "lsoda")
end_time <- Sys.time()# End timer
(duration <- end_time - start_time)

# I.Process results_Palestine
results_Palestine <- as.data.frame(output_Palestine)
...
colnames(results_Palestine) <- col_names
names(results_Palestine)
...
total_pop<- rowSums(results_Palestine[, 2:(20*n_age)])  # This will exclude time column
...
ggplot(P_t, aes(x = tim, y = total_pop)) +
  ...
  labs(
    title = "Total population over time in Palestine",
...
#Prob.susceptible
#Susceptible
results_Palestine$susceptible <- rowSums( 
  results_Palestine[, (Sindex + 1)])+
  rowSums(results_Palestine[, (V1Sindex + 1)])+
  rowSums(results_Palestine[, (V2Sindex + 1)])+
  rowSums(results_Palestine[, (SIASindex + 1)])
prop_sus_t<-(results_Palestine$susceptible/total_pop)
...
ggplot(P_t, aes(x = tim, y = prop_sus_t)) +
  ...
  geom_line(color = "blue") +
  labs(
    title = "Total susceptible over time in Palestine",
...
#
# 7.Reshape results_Palestine for data analysis
names(results_Palestine)
time_index <- which(names(results_Palestine) == "time")
# Select "time" and next columns 20*101
20*101
selected <- results_Palestine[, c(time_index, (time_index + 1):(time_index + 20*n_age))]
column_names <- names(selected)
results_Palestine_0<-results_Palestine[column_names]
#8.Long format of model results_Palestine
#results_Palestine<-results_Palestine[,1:((16*n_age)+1)] #12 are SIR, 4 others are H0,H1,H2,H3
pacman::p_load(data.table) # This package will allow us to reshape dataset faster
results_Palestine_0<- as.data.table(results_Palestine_0)
#Long format using melt to be faster
results_Palestine_long <- melt(
  results_Palestine_0,
  id.vars = "time",
...
names(results_Palestine_long)
head(results_Palestine_long)
table(results_Palestine_long$compartment)
#Here i will need to speed the visualization 
pacman::p_load(data.table,ggplot2,scales)
# Data.table 
setDT(results_Palestine_long)
#Transformations in one data.table 
results_Palestine_long_optimized <- results_Palestine_long[, `:=`(
...
results_Palestine_long_optimized[as.integer(age_group) <= 101, `:=`(
  total_by_age = sum(value), 
  proportion = value / sum(value) * 100
), by = age_group]

#Immunity column 
results_Palestine_long_optimized[, Immunity := fcase(
...
# Levels efficiently
results_Palestine_long_optimized[, Vaccine := factor(Vaccine, levels = c("0-Dose", "1-Dose", "2-Doses", "SIA-Dose"))]
# FUNCTIONAL IMMUNITY VISUALIZATION

# Pre-aggregate first
Final_Palestine_immunity_summary <- results_Palestine_long_optimized[
...
names(Final_Palestine_immunity_summary)
dim(Final_Palestine_immunity_summary)
immunity1 <- ggplot(Final_Palestine_immunity_summary, 
                    aes(x = age_group, y = proportion, fill = Immunity)) +
  geom_col(position = "stack") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 8)) +
  labs(title = "Functional Immunity in Palestine", x = "Age", y = "Percent of the population") +
  scale_fill_manual(values = c("MCV1" = "#00c4aa", "MCV2" = "#e573f3", 
                               "Natural" = "#00b3f4", "SIA" = "#9b9602", "Non-immune" = "#fc726c"))
print(immunity1)
# Save
ggsave("Final_immunity_Palestine_stacked_plot.png", plot = immunity1, width = 10, height = 10)
#Data to be used to compare countries
write.csv(Final_Palestine_immunity_summary, "Final_Palestine_immunity_summary.csv", row.names = FALSE)
# Fast file writing
fwrite(results_Palestine_long_optimized, "Final_FI_Palestine.csv")
# Fast image saving
ggsave("3.U.1.Immunity_stack_Palestine_0.png", plot = immunity1, 
       width = 10, height = 10, dpi = 300)

# Pre-aggregate data  (for dodged plot)
immunity2_summary <- results_Palestine_long_optimized[
...
print(immunity2)
# Save
ggsave("Final_immunity_Palestine_dodged_plot.png", plot = immunity2, width = 10, height = 10)

#
#Age
table(results_Palestine_long$age_group)
#Compartments
table(results_Palestine_long$compartment)
(results_Palestine_long1 <- results_Palestine_long %>%
    mutate(
      Status = case_when(
        compartment %in% c("I", "V1I", "V2I", "SIAI","H0", "H1", "H2","H3") ~ "Infectious",
        compartment %in% c("E", "V1E", "V2E", "SIAE") ~ "Exposed",
...
head(results_Palestine_long1)
results_Palestine_long1
names(results_Palestine_long1)
head(results_Palestine_long1)
#Vaccination status 
vaccination_status<-ggplot(results_Palestine_long1 |>
                             filter(age_group %in% levels(age_group)[1:50]),
                           aes(x = age_group, y = value, fill = Vaccine)) +
  geom_col(position = "stack") +
  labs(
    title = "Vaccination Status of Population in Palestine",
    x = "Age Group",
...
end <- max(results_Palestine_long1$time_days)
#
final_vaccination <- results_Palestine_long1 %>%
...
parameters
#Checks
table(results_Palestine_long1$Status)
table(results_Palestine_long1$Vaccine)
#Packages for visualization
...
infectious_bar_plot1_a <- ggplot(results_Palestine_long1 %>%
...
infectious_bar_plot1_b <- ggplot(results_Palestine_long1 %>%
  ...
  labs(x = "Age group", y = "Number Infectious",
       title = "Measles cases and vaccination status in Palestine") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
calculate_effective_Rt_parallel <- function(results_Palestine, parameters) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Palestine[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Palestine$time,
    time_years = results_Palestine$time / 365,
    R_eff = unlist(R_eff)
  )
}
start_time <- Sys.time()
Rt_data_Palestine <- calculate_effective_Rt_parallel(results_Palestine, parameters)
end_time <- Sys.time()
end_time - start_time
# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Palestine, aes(x = time_years, y = R_eff)) +
    ...
    annotate("text", x = 2, y = max(Rt_data_Palestine$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Palestine$R_eff) * 0.8, 
...
(max_Reff<-max(Rt_data_Palestine$R_eff))
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Palestine$year <- 2000 + Rt_data_Palestine$time_years
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Palestine, aes(x = time_years, y = R_eff)) +
    ...
    geom_point(data = Rt_data_Palestine[seq(1, nrow(Rt_data_Palestine), by = 50), ], 
               color = "#E63946", size = 1) +
    ...
    # Text annotations
    annotate("text", x = max(Rt_data_Palestine$time_years) * 0.8, y = max(Rt_data_Palestine$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Palestine$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Palestine$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    
    labs(
      x = "Time (Years)",
      y = "Rt",
      title = "Epidemic control assessment in Palestine :Baseline",
      #subtitle = "Green:disease control (Rt < 1)|Red :Potential epidemic spread (Rt > 1)"
    ) +
    scale_y_continuous(limits = c(0, max(Rt_data_Palestine$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
#Year in thousand
Rt_data_Palestine$year <- as.numeric(2000 + Rt_data_Palestine$time_years)

Rt_zone_plot <- ggplot(Rt_data_Palestine, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Palestine$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
  ...
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Palestine$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Palestine$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Palestine$R_eff) * 0.1, 
...
(Rt_hist_plot <- ggplot(Rt_data_Palestine, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Palestine$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Palestine$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Palestine$R_eff) + 0.1, y = max(table(cut(Rt_data_Palestine$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Palestine$R_eff), 2)), 
    ...
    labs(
      ...
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Palestine$R_eff < 1) / nrow(Rt_data_Palestine), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
vaccine_coverage_over_time <- results_Palestine_long1 %>%
...
combined_data <- Rt_data_Palestine %>%
...
(Rt_coverage_plot <- ggplot(combined_data, aes(x = time_years)) +
    ...
    labs(
      x = "Time (Years)",
      title = "Rt and vaccination coverage over time in Palestine"
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
start_time <- Sys.time()
output_Palestine_scenario_1 <- ode(y = Y,times = times,func = Palestine_model,parms = parameters_1,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Palestine
results_Palestine_scenario_1 <- as.data.frame(output_Palestine_scenario_1)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Palestine_scenario_1) <- col_names
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
calculate_effective_Rt_parallel <- function(results_Palestine_scenario_1, parameters_1) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Palestine_scenario_1[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Palestine_scenario_1$time,
    time_years = results_Palestine_scenario_1$time / 365,
    R_eff = unlist(R_eff)
  )
}
...
Rt_data_Palestine_scenario_1 <- calculate_effective_Rt_parallel(results_Palestine_scenario_1, parameters_1)
end_time <- Sys.time() #End
end_time - start_time  #Duration
# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_1, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Palestine_scenario_1$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Palestine_scenario_1$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
#
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Palestine_scenario_1$year <- 2000 + Rt_data_Palestine_scenario_1$time_years

# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_1, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Palestine_scenario_1[seq(1, nrow(Rt_data_Palestine_scenario_1), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Palestine_scenario_1$time_years) * 0.8, y = max(Rt_data_Palestine_scenario_1$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 2, fontface = "bold") +
    annotate("text", x = max(Rt_data_Palestine_scenario_1$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 2, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Palestine_scenario_1$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    
    labs(
      x = "Time (Years)",
      y = "Rt",
      title = "Epidemic control assessment in Palestine :SIA-RI",
      #subtitle = "Green:disease control (Rt < 1)|Red :Potential epidemic spread (Rt > 1)"
    ) +
    scale_y_continuous(limits = c(0, max(Rt_data_Palestine_scenario_1$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Palestine_scenario_1$year <- as.numeric(2000 + Rt_data_Palestine_scenario_1$time_years)

Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_1, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Palestine_scenario_1$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Palestine_scenario_1[seq(1, nrow(Rt_data_Palestine_scenario_1), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Palestine_scenario_1$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Palestine_scenario_1$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Palestine_scenario_1$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
Rt_zone_plot
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Palestine_scenario_1, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Palestine_scenario_1$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Palestine_scenario_1$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Palestine_scenario_1$R_eff) + 0.1, y = max(table(cut(Rt_data_Palestine$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Palestine_scenario_1$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      ...
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Palestine_scenario_1$R_eff < 1) / nrow(Rt_data_Palestine_scenario_1), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
end_time <- Sys.time()
output_Palestine_scenario_2 <- ode(y = Y,times = times,func = Palestine_model,parms = parameters_2,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Palestine
results_Palestine_scenario_2 <- as.data.frame(output_Palestine_scenario_2)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Palestine_scenario_2) <- col_names
#Rt-2
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Epidemic control assessment  using Rt 
...
calculate_effective_Rt_parallel <- function(results_Palestine_scenario_2, parameters_2) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Palestine_scenario_2[, idx(offset)])
  }
  ...
  # Result
  data.frame(
    time_days = results_Palestine_scenario_2$time,
    time_years = results_Palestine_scenario_2$time / 365,
    R_eff = unlist(R_eff)
  )
}
...
#  Rt data
start_time <- Sys.time()
Rt_data_Palestine_scenario_2 <- calculate_effective_Rt_parallel(results_Palestine_scenario_2,parameters_2)
end_time <- Sys.time()
end_time - start_time

# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_2, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
#
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Palestine_scenario_2$year <- 2000 + Rt_data_Palestine_scenario_2$time_years

# Plot using calendar years on x-axis
Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_2, aes(x = year, y = R_eff)) +
  geom_line(color = "#E63946", size = 1.2) +
  ...
  # Annotations
  annotate("text", x = 2005, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.9, 
           label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
  annotate("text", x = 2005, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.8, 
           label = "SIA campaigns", color = "blue", size = 3) +
...
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_2, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Palestine_scenario_2[seq(1, nrow(Rt_data_Palestine_scenario_2), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Palestine_scenario_2$time_years) * 0.8, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Palestine_scenario_2$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    ...
    scale_y_continuous(limits = c(0, max(Rt_data_Palestine_scenario_2$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Palestine_scenario_2$year <- as.numeric(2000 + Rt_data_Palestine_scenario_2$time_years)

Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_2, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Palestine_scenario_2$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Palestine_scenario_2[seq(1, nrow(Rt_data_Palestine_scenario_2), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Palestine_scenario_2$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
#plotly::ggplotly(Rt_zone_plot)
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Palestine_scenario_2, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Palestine_scenario_2$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Palestine_scenario_2$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Palestine_scenario_2$R_eff) + 0.1, y = max(table(cut(Rt_data_Palestine_scenario_2$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Palestine_scenario_2$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      ...
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Palestine_scenario_2$R_eff < 1) / nrow(Rt_data_Palestine_scenario_2), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
names(vaccine_coverage_over_time)
# Rt and vaccination data
combined_data <- Rt_data_Palestine_scenario_2 %>%
  left_join(vaccine_coverage_over_time, by = c("time_days", "time_years"))
...
output_Palestine_scenario_3 <- ode(y = Y,times = times,func = Palestine_model,parms = parameters_3,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Palestine
results_Palestine_scenario_3 <- as.data.frame(output_Palestine_scenario_3)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Palestine_scenario_3) <- col_names
...
calculate_effective_Rt_parallel <- function(results_Palestine_scenario_3, parameters_3) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Palestine_scenario_3[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Palestine_scenario_3$time,
    time_years = results_Palestine_scenario_3$time / 365,
    R_eff = unlist(R_eff)
  )
}
...
#  Rt data
start_time<-Sys.time()
(Rt_data_Palestine_scenario_3 <- calculate_effective_Rt_parallel(results_Palestine_scenario_3, parameters_3))
end_time<-Sys.time()
(end_time-start_time)

# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_3, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
#
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Palestine_scenario_3$year <- 2000 + Rt_data_Palestine_scenario_3$time_years
dim(Rt_data_Palestine_scenario_3)
# Plot using calendar years on x-axis
Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_3, aes(x = year, y = R_eff)) +
  geom_line(color = "#E63946", size = 1.2) +
  ...
  # Annotations
  annotate("text", x = 2005, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.9, 
           label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
  annotate("text", x = 2005, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.8, 
           label = "SIA campaigns", color = "blue", size = 3) +
...
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_3, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Palestine_scenario_3[seq(1, nrow(Rt_data_Palestine_scenario_3), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Palestine_scenario_3$time_years) * 0.8, y = max(Rt_data_Palestine_scenario_2$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Palestine_scenario_3$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    ...
    scale_y_continuous(limits = c(0, max(Rt_data_Palestine_scenario_3$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Palestine_scenario_3$year <- as.numeric(2000 + Rt_data_Palestine_scenario_3$time_years)

Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_3, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Palestine_scenario_3$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Palestine_scenario_3[seq(1, nrow(Rt_data_Palestine_scenario_3), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Palestine_scenario_3$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Palestine_scenario_3$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
Rt_zone_plot
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Palestine_scenario_3, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Palestine_scenario_3$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Palestine_scenario_3$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Palestine_scenario_3$R_eff) + 0.1, y = max(table(cut(Rt_data_Palestine_scenario_3$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Palestine_scenario_3$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      title = "Distribution of Rt values",
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Palestine_scenario_3$R_eff < 1) / nrow(Rt_data_Palestine_scenario_3), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
names(vaccine_coverage_over_time)
# Rt and vaccination data
combined_data <- Rt_data_Palestine_scenario_3 %>%
  left_join(vaccine_coverage_over_time, by = c("time_days", "time_years"))
...
#Timer
end_time <- Sys.time()
output_Palestine_scenario_4 <- ode(y = Y,times = times,func = Palestine_model,parms = parameters_4,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results_Palestine
results_Palestine_scenario_4 <- as.data.frame(output_Palestine_scenario_4)
# Column names (including hospitalization and wasted doses compartments and death counters
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Palestine_scenario_4) <- col_names
...
calculate_effective_Rt_parallel <- function(results_Palestine_scenario_4, parameters_4) {
  library(parallel)
  ...
  get_compartment <- function(offset) {
    as.matrix(results_Palestine_scenario_4[, idx(offset)])
  }
  ...
  # Return result
  data.frame(
    time_days = results_Palestine_scenario_4$time,
    time_years = results_Palestine_scenario_4$time / 365,
    R_eff = unlist(R_eff)
  )
}
# Rt data
start_time<-Sys.time()
(Rt_data_Palestine_scenario_4 <- calculate_effective_Rt_parallel(results_Palestine_scenario_4, parameters_4))
end_time<-Sys.time()
(end_time-start_time)

# Plot 1: Basic R_eff over time
(Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_4, aes(x = time_years, y = R_eff)) +
    geom_line(color = "#E63946", size = 1.2) +
    geom_hline(yintercept = 1, linetype = "dashed", color = "black", alpha = 0.7, size = 1) +
    geom_vline(xintercept = seq(0, 3, by = 2), linetype = "dotted", color = "blue", alpha = 0.5) +
    annotate("text", x = 2, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.9, 
             label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
    annotate("text", x = 1, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.8, 
             label = "SIA campaigns", color = "blue", size = 3) +
    labs(
      x = "Time (Years)",
      y = "Rt",
...
# Adjusted: time_years to actual calendar years starting at 2000
Rt_data_Palestine_scenario_4$year <- 2000 + Rt_data_Palestine_scenario_4$time_years

# Plot using calendar years on x-axis
Rt_basic_plot <- ggplot(Rt_data_Palestine_scenario_4, aes(x = year, y = R_eff)) +
  geom_line(color = "#E63946", size = 1.2) +
  ...
  # Annotations
  annotate("text", x = 2005, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.9, 
           label = "Rt = 1 (Epidemic threshold)", color = "black", size = 3.5) +
  annotate("text", x = 2005, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.8, 
           label = "SIA campaigns", color = "blue", size = 3) +
...
Rt_basic_plot
# Plot 2: R_eff with epidemic threshold zones
(Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_4, aes(x = time_years, y = R_eff)) +
    # Colored background zones
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
             fill = "#ffcccc", alpha = 0.3) +  # Epidemic zone (R > 1)
    annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
    ...
    geom_line(color = "#E63946", size = 1.3) +
    geom_point(data = Rt_data_Palestine_scenario_4[seq(1, nrow(Rt_data_Palestine_scenario_4), by = 50), ], 
               color = "#E63946", size = 1) +
    geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
    geom_vline(xintercept = seq(0, 2, by = 2), linetype = "dashed", color = "blue", alpha = 0.6) +
    
    # Text annotations
    annotate("text", x = max(Rt_data_Palestine_scenario_4$time_years) * 0.8, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.95, 
             label = "Epidemic Zone:(Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
    annotate("text", x = max(Rt_data_Palestine_scenario_4$time_years) * 0.8, y = 0.5, 
             label = "Control Zone:(Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
    annotate("text", x = 4, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.1, 
             label = "SIA campaigns every 2 years", color = "blue", size = 3, angle = 0) +
    ...
    scale_y_continuous(limits = c(0, max(Rt_data_Palestine_scenario_4$R_eff) * 1.1)) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
      plot.subtitle = element_text(size = 11),
...
#Year in thousand
Rt_data_Palestine_scenario_4$year <- as.numeric(2000 + Rt_data_Palestine_scenario_4$time_years)

Rt_zone_plot <- ggplot(Rt_data_Palestine_scenario_4, aes(x = year, y = R_eff)) +
  scale_x_continuous(breaks = seq(2000, max(Rt_data_Palestine_scenario_4$year), by = 5)) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = 1, ymax = Inf, 
           fill = "#ffcccc", alpha = 0.3) +
  annotate("rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 1, 
           fill = "#ccffcc", alpha = 0.3) +
  ...
  geom_point(data = Rt_data_Palestine_scenario_4[seq(1, nrow(Rt_data_Palestine_scenario_4), by = 50), ], 
             aes(x = year, y = R_eff), color = "#E63946", size = 1) +
  geom_hline(yintercept = 1, linetype = "solid", color = "black", size = 1) +
  geom_vline(xintercept = seq(2023, max(Rt_data_Palestine_scenario_4$year), by = 3), 
             linetype = "dashed", color = "blue", alpha = 0.6) +
  annotate("text", x = 2020, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.95, 
           label = "Epidemic Zone: (Rt > 1)", color = "#8B0000", size = 4, fontface = "bold") +
  annotate("text", x = 2020, y = 0.5, 
           label = "Control Zone: (Rt < 1)", color = "#006400", size = 4, fontface = "bold") +
  annotate("text", x = 2004, y = max(Rt_data_Palestine_scenario_4$R_eff) * 0.1, 
           label = "SIA campaigns every 2 years", color = "blue", size = 3) +
  labs(
    x = "Year",
    y = "Rt",
...
#plotly::ggplotly(Rt_zone_plot)
# Plot 3: R_eff distribution histogram
(Rt_hist_plot <- ggplot(Rt_data_Palestine_scenario_4, aes(x = R_eff)) +
    geom_histogram(bins = 30, fill = "#118AB2", alpha = 0.7, color = "white") +
    geom_vline(xintercept = 1, linetype = "dashed", color = "#E63946", size = 1.2) +
    geom_vline(xintercept = mean(Rt_data_Palestine_scenario_4$R_eff), linetype = "solid", color = "orange", size = 1) +
    annotate("text", x = 1.1, y = max(table(cut(Rt_data_Palestine_scenario_4$R_eff, breaks = 30))) * 0.8,
             label = "Epidemic\nthreshold", color = "#E63946", size = 3.5, hjust = 0) +
    annotate("text", x = mean(Rt_data_Palestine_scenario_4$R_eff) + 0.1, y = max(table(cut(Rt_data_Palestine_scenario_4$R_eff, breaks = 30))) * 0.6,
             label = paste("Mean =", round(mean(Rt_data_Palestine_scenario_4$R_eff), 2)), 
             color = "orange", size = 3.5, hjust = 0) +
    labs(
      x = "Rt",
      y = "Frequency",
      title = "Distribution of Rt values",
      subtitle = paste("Proportion of time with Rt < 1:", 
                       round(100 * sum(Rt_data_Palestine_scenario_4$R_eff < 1) / nrow(Rt_data_Palestine_scenario_4), 1), "%")
    ) +
    theme_minimal() +
    theme(
      plot.title = element_text(size = 14, face = "bold"),
...
names(vaccine_coverage_over_time)
# Rt and vaccination data
combined_data <- Rt_data_Palestine_scenario_4 %>%
  left_join(vaccine_coverage_over_time, by = c("time_days", "time_years"))
...
# Timer
start_time <- Sys.time()
output_Palestine_scenario_5 <- ode(y = Y, times = times,func = Palestine_model,parms = parameters_5,method = "lsoda")
end_time <- Sys.time()
(duration <- end_time - start_time)
# Process results
results_Palestine_scenario_5 <- as.data.frame(output_Palestine_scenario_5)
# Rename columns
compartment_names <- c("S", "E", "I", "R", "V1S", "V1E", "V1I", "V1R", "V2S", "V2E", "V2I", "V2R", 
                       "SIAS", "SIAE", "SIAI", "SIAR", "H0", "H1", "H2", "H3",
                       "D0", "D1", "D2", "D3", "DTotal",
...
colnames(results_Palestine_scenario_5) <- col_names

# 2025-2035 index
index_2025<-365*25
index_2035<-nrow(results_Palestine_scenario_4)
#New subsets
#results_Palestine<-results_Palestine[index_2025:index_2035,]
#results_Palestine_scenario_1<-results_Palestine_scenario_1[index_2025:index_2035,]
#results_Palestine_scenario_2<-results_Palestine_scenario_2[index_2025:index_2035,]
#results_Palestine_scenario_3<-results_Palestine_scenario_3[index_2025:index_2035,]
#results_Palestine_scenario_4<-results_Palestine_scenario_4[index_2025:index_2035,]
#results_Palestine_scenario_5<-results_Palestine_scenario_5[index_2025:index_2035,]

head(results_Palestine)
...
results_Palestine <- as.data.frame(results_Palestine)
results_Palestine$scenario <- "RI"
results_Palestine$Country <- "Palestine"

# 0.a. Incidence
results_Palestine$incidence <- report_sigma * (
  rowSums(results_Palestine[, Eindex + 1]) +
    rowSums(results_Palestine[, V1Eindex + 1]) +
    rowSums(results_Palestine[, V2Eindex + 1]) +
    rowSums(results_Palestine[, SIAEindex + 1])
)

# 0.b. Cumulative incidence
results_Palestine$cum_incidence <- cumsum(results_Palestine$incidence)

# 0.c. Severe cases
results_Palestine$hospitalization <- rowSums(results_Palestine[, H0index + 1]) +
  rowSums(results_Palestine[, H1index + 1]) +
  rowSums(results_Palestine[, H2index + 1]) +
  rowSums(results_Palestine[, H3index + 1])

# 0.d. Cumulative severe cases
results_Palestine$cum_hospitalization <- cumsum(results_Palestine$hospitalization)

# 0.e. Deaths
results_Palestine$deaths <- rowSums(results_Palestine[, DTotalindex + 1])



# 0.f. Cumulative deaths
results_Palestine$cum_deaths <- cumsum(results_Palestine$deaths)
summary(results_Palestine$cum_death)
summary(results_Palestine$deaths)

# 0.g. Wasted doses
results_Palestine$wasted_doses <-rowSums(results_Palestine[, W3index + 1])

# 0.h. Cumulative wasted doses
results_Palestine$cum_wasted_doses <- cumsum(results_Palestine$wasted_doses)


#Scenario 1 :"RI+SIA-2Y"
results_Palestine_scenario_1 <- as.data.frame(results_Palestine_scenario_1)
results_Palestine_scenario_1$scenario <- "RI+SIA-2Y"
results_Palestine_scenario_1$Country <- "Palestine"

# 1.a. Incidence
results_Palestine_scenario_1$incidence <- report_sigma * (
  rowSums(results_Palestine_scenario_1[, Eindex + 1]) +
    rowSums(results_Palestine_scenario_1[, V1Eindex + 1]) +
    rowSums(results_Palestine_scenario_1[, V2Eindex + 1]) +
    rowSums(results_Palestine_scenario_1[, SIAEindex + 1])
)

# 1.b. Cumulative incidence
results_Palestine_scenario_1$cum_incidence <- cumsum(results_Palestine_scenario_1$incidence)

# 1.c. Severe cases
results_Palestine_scenario_1$hospitalization <- rowSums(results_Palestine_scenario_1[, H0index + 1]) +
  rowSums(results_Palestine_scenario_1[, H1index + 1]) +
  rowSums(results_Palestine_scenario_1[, H2index + 1]) +
  rowSums(results_Palestine_scenario_1[, H3index + 1])

# 1.d. Cumulative severe cases
results_Palestine_scenario_1$cum_hospitalization <- cumsum(results_Palestine_scenario_1$hospitalization)

# 1.e. Deaths
results_Palestine_scenario_1$deaths <- rowSums(results_Palestine_scenario_1[, DTotalindex + 1])

# 1.f. Cumulative deaths
results_Palestine_scenario_1$cum_deaths <- cumsum(results_Palestine_scenario_1$deaths)

# 1.g. Wasted doses
results_Palestine_scenario_1$wasted_doses <-rowSums(results_Palestine_scenario_1[, W3index + 1])

# 1.h. Cumulative wasted doses
results_Palestine_scenario_1$cum_wasted_doses <- cumsum(results_Palestine_scenario_1$wasted_doses)

#Scenario 2 :RI-Scaled-up
results_Palestine_scenario_2 <- as.data.frame(results_Palestine_scenario_2)
results_Palestine_scenario_2$scenario <- "RI-Scaled-up"
results_Palestine_scenario_2$Country <- "Palestine"

# 2.a. Incidence
results_Palestine_scenario_2$incidence <- report_sigma * (
  rowSums(results_Palestine_scenario_2[, Eindex + 1]) +
    rowSums(results_Palestine_scenario_2[, V1Eindex + 1]) +
    rowSums(results_Palestine_scenario_2[, V2Eindex + 1]) +
    rowSums(results_Palestine_scenario_2[, SIAEindex + 1])
)

# 2.b. Cumulative incidence
results_Palestine_scenario_2$cum_incidence <- cumsum(results_Palestine_scenario_2$incidence)

# 2.c. Severe cases
results_Palestine_scenario_2$hospitalization <- rowSums(results_Palestine_scenario_2[, H0index + 1]) +
  rowSums(results_Palestine_scenario_2[, H1index + 1]) +
  rowSums(results_Palestine_scenario_2[, H2index + 1]) +
  rowSums(results_Palestine_scenario_2[, H3index + 1])

# 2.d. Cumulative severe cases
results_Palestine_scenario_2$cum_hospitalization <- cumsum(results_Palestine_scenario_2$hospitalization)

# 2.e. Deaths
results_Palestine_scenario_2$deaths <- rowSums(results_Palestine_scenario_2[, DTotalindex + 1])

# 2.f. Cumulative deaths
results_Palestine_scenario_2$cum_deaths <- cumsum(results_Palestine_scenario_2$deaths)

# 2.g. Wasted doses
results_Palestine_scenario_2$wasted_doses <-rowSums(results_Palestine_scenario_2[, W3index + 1])

# 2.h. Cumulative wasted doses
results_Palestine_scenario_2$cum_wasted_doses <- cumsum(results_Palestine_scenario_2$wasted_doses)


#Scenario 3:"RI+SIA-5Y"
results_Palestine_scenario_3 <- as.data.frame(results_Palestine_scenario_3)
results_Palestine_scenario_3$scenario <- "RI+SIA-5Y"
results_Palestine_scenario_3$Country <- "Palestine"

# 3.a. Incidence
results_Palestine_scenario_3$incidence <- report_sigma * (
  rowSums(results_Palestine_scenario_3[, Eindex + 1]) +
    rowSums(results_Palestine_scenario_3[, V1Eindex + 1]) +
    rowSums(results_Palestine_scenario_3[, V2Eindex + 1]) +
    rowSums(results_Palestine_scenario_3[, SIAEindex + 1])
)

# 3.b. Cumulative incidence
results_Palestine_scenario_3$cum_incidence <- cumsum(results_Palestine_scenario_3$incidence)

# 3.c. Severe cases
results_Palestine_scenario_3$hospitalization <- rowSums(results_Palestine_scenario_3[, H0index + 1]) +
  rowSums(results_Palestine_scenario_3[, H1index + 1]) +
  rowSums(results_Palestine_scenario_3[, H2index + 1]) +
  rowSums(results_Palestine_scenario_3[, H3index + 1])

# 3.d. Cumulative severe cases
results_Palestine_scenario_3$cum_hospitalization <- cumsum(results_Palestine_scenario_3$hospitalization)

# 3.e. Deaths
results_Palestine_scenario_3$deaths <- rowSums(results_Palestine_scenario_3[, DTotalindex + 1])

# 3.f. Cumulative deaths
results_Palestine_scenario_3$cum_deaths <- cumsum(results_Palestine_scenario_3$deaths)

# 3.g. Wasted doses
results_Palestine_scenario_3$wasted_doses <- rowSums(results_Palestine_scenario_3[, W3index + 1])

# 3.h. Cumulative wasted doses
results_Palestine_scenario_3$cum_wasted_doses <- cumsum(results_Palestine_scenario_3$wasted_doses)
# Scenario 4: "RI-Reduced"
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~>
results_Palestine_scenario_4 <- as.data.frame(results_Palestine_scenario_4)
results_Palestine_scenario_4$scenario <- "RI-Reduced"
results_Palestine_scenario_4$Country <- "Palestine"

# 4.a. Incidence
results_Palestine_scenario_4$incidence <- report_sigma * (
  rowSums(results_Palestine_scenario_4[, Eindex + 1]) +
    rowSums(results_Palestine_scenario_4[, V1Eindex + 1]) +
    rowSums(results_Palestine_scenario_4[, V2Eindex + 1]) +
    rowSums(results_Palestine_scenario_4[, SIAEindex + 1])
)

# 4.b. Cumulative incidence
results_Palestine_scenario_4$cum_incidence <- cumsum(results_Palestine_scenario_4$incidence)

# 4.c. Severe cases
results_Palestine_scenario_4$hospitalization <- rowSums(results_Palestine_scenario_4[, H0index + 1]) +
  rowSums(results_Palestine_scenario_4[, H1index + 1]) +
  rowSums(results_Palestine_scenario_4[, H2index + 1]) +
  rowSums(results_Palestine_scenario_4[, H3index + 1])

# 4.d. Cumulative severe cases
results_Palestine_scenario_4$cum_hospitalization <- cumsum(results_Palestine_scenario_4$hospitalization)

# 4.e. Deaths
results_Palestine_scenario_4$deaths <- rowSums(results_Palestine_scenario_4[, DTotalindex + 1])

# 4.f. Cumulative deaths
results_Palestine_scenario_4$cum_deaths <- cumsum(results_Palestine_scenario_4$deaths)

# 4.g. Wasted doses
results_Palestine_scenario_4$wasted_doses <-rowSums(results_Palestine_scenario_4[, W3index + 1])

# 4.h. Cumulative wasted doses
results_Palestine_scenario_4$cum_wasted_doses <- cumsum(results_Palestine_scenario_4$wasted_doses)

#Scenario 5.Pre-screened vaccination (Routine,SIA)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~>
results_Palestine_scenario_5 <- as.data.frame(results_Palestine_scenario_5)
results_Palestine_scenario_5$scenario <- "Pre-screening"
results_Palestine_scenario_5$Country <- "Palestine"

# 5.a. Incidence
results_Palestine_scenario_5$incidence <- report_sigma * (
  rowSums(results_Palestine_scenario_5[, Eindex + 1]) +
    rowSums(results_Palestine_scenario_5[, V1Eindex + 1]) +
    rowSums(results_Palestine_scenario_5[, V2Eindex + 1]) +
    rowSums(results_Palestine_scenario_5[, SIAEindex + 1])
)

# 5.b. Cumulative incidence
results_Palestine_scenario_5$cum_incidence <- cumsum(results_Palestine_scenario_5$incidence)

# 5.c. Severe cases
results_Palestine_scenario_5$hospitalization <- rowSums(results_Palestine_scenario_5[, H0index + 1]) +
  rowSums(results_Palestine_scenario_5[, H1index + 1]) +
  rowSums(results_Palestine_scenario_5[, H2index + 1]) +
  rowSums(results_Palestine_scenario_5[, H3index + 1])

# 5.d. Cumulative severe cases
results_Palestine_scenario_5$cum_hospitalization <- cumsum(results_Palestine_scenario_5$hospitalization)

# 5.e. Deaths
results_Palestine_scenario_5$deaths <- rowSums(results_Palestine_scenario_5[, DTotalindex + 1])

# 5.f. Cumulative deaths
results_Palestine_scenario_5$cum_deaths <- cumsum(results_Palestine_scenario_5$deaths)

# 5.g. Wasted doses
results_Palestine_scenario_5$wasted_doses <- rowSums(results_Palestine_scenario_5[, W1index + 1]) +
  rowSums(results_Palestine_scenario_5[, W2index + 1]) +
  rowSums(results_Palestine_scenario_5[, W3index + 1])

# 5.h. Cumulative wasted doses
results_Palestine_scenario_5$cum_wasted_doses <- cumsum(results_Palestine_scenario_5$wasted_doses)


# Checks
dim(results_Palestine)
dim(results_Palestine_scenario_1)
dim(results_Palestine_scenario_2)
dim(results_Palestine_scenario_3)
dim(results_Palestine_scenario_4)
dim(results_Palestine_scenario_5)

# Impact of interventions/secnarios 
# Incidence

(scenario_1_cum_incidence <- (tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_1$cum_incidence, 1)) / tail(results_Palestine$cum_incidence, 1) * 100)
(scenario_2_cum_incidence <- (tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_2$cum_incidence, 1)) / tail(results_Palestine$cum_incidence, 1) * 100)
(scenario_3_cum_incidence <- (tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_3$cum_incidence, 1)) / tail(results_Palestine$cum_incidence, 1) * 100)
(scenario_4_cum_incidence <- (tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_4$cum_incidence, 1)) / tail(results_Palestine$cum_incidence, 1) * 100)

#Severe cases 
(scenario_1_cum_hospitalization <- (tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_1$cum_hospitalization, 1)) / tail(results_Palestine$cum_hospitalization, 1) * 100)
(scenario_2_cum_hospitalization <- (tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_2$cum_hospitalization, 1)) / tail(results_Palestine$cum_hospitalization, 1) * 100)
(scenario_3_cum_hospitalization <- (tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_3$cum_hospitalization, 1)) / tail(results_Palestine$cum_hospitalization, 1) * 100)
(scenario_4_cum_hospitalization <- (tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_4$cum_hospitalization, 1)) / tail(results_Palestine$cum_hospitalization, 1) * 100)
#Deaths
(scenario_1_cum_deaths <- (tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_1$cum_deaths, 1)) / tail(results_Palestine$cum_deaths, 1) * 100)
(scenario_2_cum_deaths <- (tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_2$cum_deaths, 1)) / tail(results_Palestine$cum_deaths, 1) * 100)
(scenario_3_cum_deaths <- (tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_4$cum_deaths, 1)) / tail(results_Palestine$cum_deaths, 1) * 100)
(scenario_4_cum_deaths <- (tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_2$cum_deaths, 1)) / tail(results_Palestine$cum_deaths, 1) * 100)


#Visualization
library(ggplot2)
#Averted cases 
averted_cases_Palestine <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SCALED-UP", "RI-SIA-5Y", "RI-REDUCED"),
  Averted_Cases = c(
    tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_1$cum_incidence, 1),
    tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_2$cum_incidence, 1),
    tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_3$cum_incidence, 1),
    tail(results_Palestine$cum_incidence, 1) - tail(results_Palestine_scenario_4$cum_incidence, 1)
  )
)
...
ggplot(averted_cases_Palestine, aes(x = Scenario, y = Averted_Cases, fill = Scenario)) +
...
write.csv(averted_cases_Palestine,"Final_averted_cases_Palestine.csv",row.names = FALSE)

#Severe cases
averted_severe_cases_Palestine <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SCALED-UP", "RI-SIA-5Y", "RI-REDUCED"),
  Averted_Severe_cases = c(
    tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_1$cum_hospitalization, 1),
    tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_2$cum_hospitalization, 1),
    tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_3$cum_hospitalization, 1),
    tail(results_Palestine$cum_hospitalization, 1) - tail(results_Palestine_scenario_4$cum_hospitalization, 1)
  )
)

library(ggplot2)
ggplot(averted_severe_cases_Palestine, aes(x = Scenario, y = Averted_Severe_cases, fill = Scenario)) +
  geom_bar(stat = "identity") +
  scale_fill_viridis_d() + 
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1))
#Save final averted deaths
write.csv(averted_severe_cases_Palestine,"Final_averted_severe_cases_Palestine.csv",row.names = FALSE)
#~~~~~~~~~~~~~~~~~#
# Averted deaths  #
#~~~~~~~~~~~~~~~~~#
averted_deaths_Palestine <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SCALED-UP", "RI-SIA-5Y", "RI-REDUCED"),
  Averted_deaths = c(
    tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_1$cum_deaths, 1),
    tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_2$cum_deaths, 1),
    tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_3$cum_deaths, 1),
    tail(results_Palestine$cum_deaths, 1) - tail(results_Palestine_scenario_4$cum_deaths, 1)
  )
)
# Plot of averted deaths
library(ggplot2)
ggplot(averted_deaths_Palestine, aes(x = Scenario, y = Averted_deaths, fill = Scenario)) +
  geom_bar(stat = "identity") +
  scale_fill_viridis_d() + 
  scale_y_continuous(labels = scales::label_number(scale = 1, accuracy = 1, suffix = " ")) +  
  labs(title = " ",
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1))
#Save final averted deaths
write.csv(averted_deaths_Palestine,"Final_averted_deaths_Palestine.csv",row.names = FALSE)
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Scenario_5: Pre-screened vaccination
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~>

averted_wasted_doses_Palestine <- data.frame(
  Scenario = c("RI-SIA-2Y", "RI-SIA-5Y"),
  Averted_wasted_doses = c(
    -tail(results_Palestine_scenario_1$cum_wasted_doses, 1) + tail(results_Palestine_scenario_5$cum_wasted_doses, 1),
    
    -tail(results_Palestine_scenario_3$cum_wasted_doses, 1) + tail(results_Palestine_scenario_5$cum_wasted_doses, 1)
  )
)
# Plot of averted_wasted_doses_Palestine
library(ggplot2)

ggplot(averted_wasted_doses_Palestine, aes(x = Scenario, y =  Averted_wasted_doses, fill = Scenario)) +
  geom_bar(stat = "identity") +
  scale_fill_viridis_d() + 
  scale_y_continuous(labels = scales::label_number(scale = 1e-6, accuracy = 1, suffix = "M ")) +  
  labs(title = " ",
  ...
  theme(axis.text.x = element_text(angle = 0, hjust = 1))
#Save final averted deaths
write.csv(averted_wasted_doses_Palestine,"Final_averted_wasted_doses_Palestine.csv",row.names = FALSE)


#results_Palestine_scenarios<-rbind(results_Palestine,
#                                     results_Palestine_scenario_1,
#                                     results_Palestine_scenario_2,
#                                     results_Palestine_scenario_3,
#                                     results_Palestine_scenario_4
#) 
#write.csv(results_Palestine_scenarios,"0.results_Palestine_scenarios.csv",row.names = FALSE)

#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# A. EPIDEMIC CONTROL ASSESSMENT: Rt data visualization
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#Visualization of Rt-data
Rt_data_Palestine$scenario<-"Baseline"
dim(Rt_data_Palestine)
Rt_data_Palestine_scenario_1$scenario<-"RI-SIA-2Y"
dim(Rt_data_Palestine_scenario_1)
Rt_data_Palestine_scenario_2$scenario<-"RI-scaled-up"
dim(Rt_data_Palestine_scenario_2)

Rt_data_Palestine_scenario_3$scenario<-"RI-SIA-5Y"
dim(Rt_data_Palestine_scenario_3)
Rt_data_Palestine_scenario_4$scenario<-"RI-Reduced"
dim(Rt_data_Palestine_scenario_4)
Rt_data_Palestine_scenarios<-rbind(Rt_data_Palestine,
                                  Rt_data_Palestine_scenario_1,
                                  Rt_data_Palestine_scenario_2,
                                  Rt_data_Palestine_scenario_3,
                                  Rt_data_Palestine_scenario_4
)

#Checks
x<-Rt_data_Palestine_scenarios
table(x$scenario)
x_0<-Rt_data_Palestine_scenarios|>
  filter(scenario=="Baseline")
x_1<-Rt_data_Palestine_scenarios|>
  filter(scenario=="RI-SIA-2Y")
x_2<-Rt_data_Palestine_scenarios|>
  filter(scenario=="RI-scaled-up")
x_3<-Rt_data_Palestine_scenarios|>
  filter(scenario=="RI-SIA-5Y")

x_4<-Rt_data_Palestine_scenarios|>
...
#Year in thousand
Rt_data_Palestine_scenarios$year <- as.numeric(2000 + Rt_data_Palestine_scenarios$time_years)
write.csv(Rt_data_Palestine_scenarios,"Final_Rt_data_Palestine_scenarios.csv",row.names = FALSE)
#Visualization
library(ggplot2)
table(Rt_data_Palestine_scenarios$scenario)
Final_Rt_Palestine<-ggplot(Rt_data_Palestine_scenarios, aes(x = year, y = R_eff, color = scenario)) +
  ...
  labs(
    title = expression("Measles time-varying " ~ R[t] ~ "in Palestine"),
    subtitle = "Palestine - Scenario Analysis",
    x = "Year",
    y = expression(R[t]),
    color = "Scenario"
  ) +
...
print(Final_Rt_Palestine)
ggsave("Final_Rt_Palestine.png",plot=Final_Rt_Palestine,height = 10, width = 12,dpi = 200 )
#At the end of each code. I saved the workenvironment in order to be re-used without re-running
#save.image(file = "Final_1_Palestine.RData")
...
# Sensitivity Analysis for Palestine Measles Model
...
run_sensitivity <- function(param_name, param_value) {
  ...
  # Run the model
  results <- ode(y = Y, times = times, func = Palestine_model, parms = new_params, method = "lsoda")
...
print(p11)
# Save the plot
ggsave("sensitivity_analysis_measles_Palestine.png",plot=p11,width=12, height=10,dpi=300)
